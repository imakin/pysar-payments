...
from .forms import WireForm
from .. import BasicProvider, RedirectNeeded, PaymentError


class WireProvider(BasicProvider):
    '''
    ...
    def get_form(self, payment, data=None):
        ...
        form = WireForm(data=data, hidden_inputs=False, provider=self,
                         payment=payment)
        if form.is_valid():
            new_status = form.cleaned_data['status']
            payment.change_status(new_status)
            new_fraud_status = form.cleaned_data['raup_status']
